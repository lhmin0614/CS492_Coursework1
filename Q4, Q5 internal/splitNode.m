...
for n = 1:iter
    ...
    %    dim2 = randi(D-1);
    %end
    %d1_min = single(min(data(:,dim1))) + eps; % Find the data range of this dimension
    %d1_max = single(max(data(:,dim1))) - eps;
    %d2_min = single(min(data(:,dim2))) + eps; % Find the data range of this dimension
    %d2_max = single(max(data(:,dim2))) - eps;
    %dmax = max(abs(d1_max - d2_min), abs(d2_max - d1_min));
    %t = rand*(dmax);
    %idx_ = abs(data(:,dim1) - data(:, dim2)) < t;
